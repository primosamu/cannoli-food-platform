...
const CampaignPreview: React.FC<CampaignPreviewProps> = ({
  ...
  const renderWhatsAppPreview = () => (
    <div className="max-w-[350px] mx-auto">
      <div className="rounded-[2rem] border-8 border-gray-900 bg-gray-100 shadow-2xl overflow-hidden">
        ...
        {/* AREA DE MENSAGENS */}
        <div className="bg-[#e5ddd5] px-3 py-4 h-[420px] overflow-y-auto">
          {/* Mensagem Exemplo */}
          ...
          {/* Pré-visualização da Mensagem com IMAGEM */}
          <div className="bg-[#DCF8C6] rounded-lg p-3 ml-auto mt-6 mb-2 shadow max-w-[85%]">
            ...
            {imageUrl && (
              <div className="mt-3 rounded-lg overflow-hidden relative w-full aspect-square bg-gray-200 flex items-center justify-center">
                <img 
                  src={imageUrl} 
                ...
                />
                {/* Exibe ícone se não houver imagem */}
                {!imageUrl && <Camera className="h-8 w-8 text-gray-400 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2" />}
              </div>
            )}
            <div className="text-[#777] text-[10px] text-right mt-1">15:30</div>
          </div>
        </div>
        {/* box de entrada de mensagem */}
  ...
  const renderSMSPreview = () => (
    <div className="max-w-[350px] mx-auto">
      <div className="rounded-[2rem] border-8 border-gray-900 bg-white shadow-2xl overflow-hidden">
        ...
        {/* Mensagem */}
        <div className="bg-white h-[425px] px-4 py-6 overflow-y-auto">
          <div className="bg-gray-100 rounded-lg p-4 mb-4 max-w-[80%]">
            <div className="text-sm whitespace-pre-line">{previewContent}</div>
            <div className="text-[10px] text-gray-700 text-right mt-2">Agora</div>
          </div>
          <div className="text-center text-xs text-muted-foreground mt-4">Prévia do SMS</div>
        </div>
      </div>
    </div>
  );
        ...
        {imageUrl && (
          <div className="w-full h-40 bg-gray-100 flex items-center justify-center mb-6 rounded">
            <img 
              ...
              onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }}
            />
            {!imageUrl && <Image className="h-8 w-8 text-gray-400" />}
  ...
  // Se não implementado: placeholder (exemplo para outros tipos)
  ...
  // Icone no header do card do preview
