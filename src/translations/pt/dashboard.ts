
import { DashboardTranslations } from "../../types/language/dashboard";

export const dashboardTranslations: DashboardTranslations = {
  // Dashboard
  overview: 'Visão Geral',
  detailedAnalytics: 'Análises Detalhadas',
  totalRevenue: 'Receita Total',
  activeCustomers: 'Clientes Ativos',
  customerSegmentation: 'Segmentação de Clientes',
  topPerformingMenuItems: 'Itens do Menu mais Vendidos',
  campaignPerformance: 'Desempenho de Campanhas',
  salesByStore: 'Vendas por Loja',
  customersAndOrdersPerDay: 'Clientes e Pedidos por Dia',
  restaurantPerformance: 'Desempenho do Restaurante',
  customerData: 'Dados de Clientes',
  
  // Common dashboard elements
  distributionAcrossDays: 'Distribuição ao longo dos dias',
  ordersPerDate: 'Pedidos por Data',
  orderForecast: 'Previsão de Pedidos',
  nextDays: 'Próximos Dias',
  historicalData: 'Dados Históricos',
  futurePredicitions: 'Previsões Futuras',
  lessThan: 'Menos',
  moreThan: 'Mais',
  customerVisits: 'Visitas de Clientes',
  trafficByHour: 'Tráfego por Hora',

  // Chart related
  orderSummary: 'Resumo de Pedidos',
  customerBehavior: 'Comportamento do Cliente',
  rfmAnalysis: 'Análise RFM',
  behaviorPatterns: 'Padrões de Comportamento',
  churnRisk: 'Risco de Abandono',
  highValue: 'Alto Valor',
  loyalCustomers: 'Clientes Fiéis',
  newCustomers: 'Novos Clientes',
  potentialLoyalists: 'Potenciais Fidelizados',
  needsAttention: 'Necessita Atenção',
  atRisk: 'Em Risco',
  cantLose: 'Não Pode Perder',
  total: 'Total',
  recency: 'Recência',
  frequency: 'Frequência',
  monetary: 'Valor Monetário',
  recentVsOlder: 'Recentes vs. Antigos',
  
  // Campaign redirection messages
  redirectingToCampaigns: 'Redirecionando para Campanhas',
  creatingCampaignFor: 'Criando campanha para',
  thisSegment: 'este segmento',
};
